...
class PseudoPopulationFolder:
    ...
    def __init__(self, k_folds, n_samples=100,
                 n_conds=1, **kwargs):
        test_prop = 1/k_folds
    ...
    def split(self, X, reindex=False):
        ...
        for i, neur in enumerate(X):
            ...
            for j, dij in enumerate(neur):
                kf_ij = skms.KFold(self.k_folds, **self.kwargs)
...
def nested_cv(c1, c2, alpha=.05, k_folds=5, n_reps=100, n_samples=100,
              pipeline=None, kernel='rbf', pre_pca=None, norm=True,
              **kwargs):
    print(n_reps, k_folds, kwargs)
    ...
    if pipeline is None:
        ...
        pipe.append(skc.SVC(kernel=kernel))
    ...
    err_outer = np.mean(e_outer, axis=2)
    a_list = (np.mean(e_inner, axis=(2, 3)) - err_outer)**2
    b_list = np.var(e_outer, axis=2)/e_outer.shape[2]
    
    mse_hat = np.mean(a_list, axis=(0, 1)) - np.mean(b_list, axis=(0, 1))
    err_hat = np.mean(e_outer, axis=(0, 1, 2))
    ...
    err_cent_all = np.mean(e_outer, axis=(1, 2))
