...
def plot_colored_pts(
    xs, ys, colors=None, ax=None, line_col=(0.9, 0.9, 0.9), linewidth=1, **kwargs
):
    if ax is None:
    ...
    for i, (x, y) in enumerate(zip(xs, ys.T)):
        ...
        plot_trace_werr(
            [x], y, points=True, fill=False, color=colors[i], ax=ax, **kwargs
        )
    return ax
            ...
            for y in range(boots):
                ...
                for j, k in enumerate(c.keys()):
                    if subsample_min is not None:
                        ...
                        n_samps = c[k].shape[0]
                    spks = u.resample_on_axis(c[k], n_samps, axis=0, with_replace=True)
...
def plot_decoding_heatmap(
    ...
    with plt.style.context(style):
        ...
        img = ax.pcolormesh(
            xs, xs, trmap, cmap=colormap, vmin=cutoff, vmax=vmax, shading="flat"
        )
        ax.set_title(title)
        if colorbar:
            f = ax.get_figure()
...
def add_color(color, add_amt):
    ...
    nc = np.min(np.stack((new_col, np.ones_like(new_col)), axis=0), axis=0)
    return nc
...
def add_color_value(color, amt):
    ...
    hsv[..., -2] = hsv[..., -2] + amt
    hsv[..., -2] = np.min(
        np.stack((np.ones_like(hsv[..., -2]), hsv[..., -2]), axis=0), axis=0
    )
    hsv[..., -2] = np.max(
...
def label_line(start, end, text, ax, buff=0, lat_offset=0, **kwargs):
    ...
    t_center_pt = np.mean(np.array((t_start, t_end)), axis=0)
...
def plot_glm_coeffs(
    ...
    ns_alpha=0.2,
    legend=True,
    legend_labels=("ns", "coeff_1", "coeff_2", "both"),
    colors=None,
):
    p_thr = p_thr / 2
...
def _cent_selectivity(inds, ps, mags, p_thr, eps, central_func=np.nanmedian):
    ps = ps[inds]
    mags = mags[inds]
    mask = np.logical_and(np.logical_or(mags > eps, mags < -eps), ps < p_thr)
    use_d = mags[mask]
...
def visualize_simplex_2d(
    ...
    **kwargs
    ...
    if plot_outline:
        ax.plot(pts_x, pts_y, "o", color="k", ms=outline_factor * ms, **kwargs)
...
def add_x_background(
    beg, end, ax, color=(0.1, 0.1, 0.1), alpha=0.1, zorder=-1, **kwargs
):
...
def get_corr_conf95(
    as_list, bs_list, n_boots=1000, func=np.corrcoef, rm_nan=False, confounders=None
):
    ...
    if rm_nan:
        mask = np.logical_or(np.isnan(as_list), np.isnan(bs_list))
...
def print_sig_fraction(
    boot_arr, subj, text, func=_greater_than_zero, p_thr=0.05, two_sided=True
):
...
def print_corr_conf95(
    ...
    cent, lower, upper, cc = get_corr_conf95(
...
def _get_sem_from_bound(err, use_lower=True, p_thr=0.05):
...
def print_mean_conf95(
    ...
    if supply_err:
        factor = sts.norm(0, 1).ppf(1 - 0.025)
        if diff:
...
def plot_stanglm_selectivity_scatter(
    ms,
    params,
    labels,
    ax=None,
    ...
    for i, m in enumerate(ms):
        ...
        for j, t in enumerate(m):
            if t is not None and (
                (not check_valid) or (check_valid and u.stan_model_valid(t))
            ):
    ...
    if format_labels:
        ax.set_xlabel(format_lm_label(params[0], labels, link_string=link_string))
        ax.set_ylabel(format_lm_label(params[1], labels, link_string=link_string))
    else:
...
def plot_glm_pop_selectivity_prop(
    coeffs,
    ps,
    subgroups=None,
    p_thr=0.05,
    ...
    use_pop, subgroups, all_use, ps = _preprocess_glm(coeffs, ps, subgroups, p_thr)
    ...
    for i, sg in enumerate(subgroups):
        ax = f.add_subplot(
            rows, len(subgroups), i + i_buff, sharey=share_ax, sharex=share_x[i]
        )
        share_ax = ax
...
def plot_glm_coeff_tc(
    ...
    for i, t in enumerate(xs):
        ...
        coeff_arr[:, i] = u.bootstrap_list(up, nm_ax, n=boots, out_shape=up.shape[1:])
    ...
    for i, sg in enumerate(subgroups):
        ...
        for j, sg_j in enumerate(sg):
            ...
            if colors is not None:
                ...
                color = None
            plot_trace_werr(xs, sg_arr, ax=ax, conf95=True, label=label, color=color)
    return axs


def plot_glm_pop_selectivity_mag(
    coeffs,
    ps,
    subgroups=None,
    p_thr=0.05,
    ...
    for i, sg in enumerate(subgroups):
        ...
        for j in sg:
            sgm = use_pop[:, j]
            psm = ps[:, j]
            mask = np.logical_and(np.logical_or(sgm > eps, sgm < -eps), psm < p_thr)
        ...
        p = ax.violinplot(
            sg_mags, positions=sg_violin, showmedians=False, showextrema=False
        )
    ...
    if group_test:
        ...
        for a, b in it.combinations(coeff_groups.keys(), 2):
            p = u.bootstrap_test(
                coeff_groups[a], coeff_groups[b], np.nanmean, n=group_test_n
            )
            test_ps[(a, b)] = p
        out = (f, test_ps)
    else:
...
def plot_glm_indiv_selectivity(
    coeffs,
    ps,
    subgroups=None,
    p_thr=0.05,
    ...
    colbar.set_ticks([round(vmin, 1) + 0.1, 0, round(vmax, 1) - 0.1])
