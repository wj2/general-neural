...
def get_population_cents_helper(ind, rf_cents, spaces, rfsizes, offsets, cents):
    for k in range(int(spaces[ind])):
        ...
        if ind < (len(spaces) - 1):
            rf_cents = get_population_cents_helper(
                ind + 1, rf_cents, spaces, rfsizes, offsets, cents
            )
        else:
...
def get_population_cents_recursive(rfsize, res, spacesize):
    ...
    for i in range(int(lat)):
        ...
        rf_cents = get_population_cents_helper(
            0, rf_cents, spaces, rfsize, offsets, cents
        )
        offsets = offsets - res
...
def plot_square_rfs(
    ...
    cm="Set3",
    cs=10,
    spacecol=(0.1, 0.1, 0.1),
    bigwid=2,
    save=False,
    savename="space_plot.svg",
):
    f = plt.figure(figsize=figsize)
    ax = f.add_subplot(1, 1, 1)
    p_space = patches.Rectangle(
        (0, 0),
        spacesize[0],
        spacesize[1],
        fill=False,
    ...
    x_min, x_max = (
        np.min(cs_arr[:, 0]) - rfsize[0] / 2.0,
        np.max(cs_arr[:, 0] + rfsize[0] / 2.0),
    )
    y_min, y_max = (
        np.min(cs_arr[:, 1]) - rfsize[1] / 2.0,
        np.max(cs_arr[:, 1]) + rfsize[1] / 2.0,
    )
...
def gaussian_ramp_func(
    coords, cents, sizes, scale, baseline, ramp_dim=None, uv=None, max_r=1
):
...
def eval_ramp_vector_rf(coords, extent, scale, baseline, sub_dim=None):
    r = eval_vector_rf(coords, ramp_func, extent, scale, baseline, sub_dim=sub_dim)
    return r


def eval_gaussian_ramp_vector_rf(
    coords, cents, sizes, scale, baseline, ramp_dim=None, uv=None, max_r=1
):
    r = eval_vector_rf(
        coords,
...
def eval_gaussian_vector_rf(coords, cents, sizes, scale, baseline, sub_dim=None):
    r = eval_vector_rf(
        coords, gaussian_func, cents, sizes, scale, baseline, sub_dim=sub_dim
    )
    return r


def eval_gaussian_vector_deriv(coords, cents, sizes, scale, baseline, sub_dim=None):
    r = eval_vector_rf(
        coords, gaussian_deriv_func, cents, sizes, scale, baseline, sub_dim=sub_dim
    )
    return r


def eval_gaussian_ramp_vector_deriv(
    coords, cents, sizes, scale, baseline, ramp_dim=None, uv=None, max_r=1
):
    r = eval_vector_rf(
        coords,
        gaussian_ramp_deriv_func,
        cents,
        sizes,
        scale,
        baseline,
...
def eval_ramp_vector_deriv(coords, extent, scale, baseline, sub_dim=None):
    r = eval_vector_rf(
        coords, ramp_deriv_func, extent, scale, baseline, sub_dim=sub_dim
    )
    return r


def get_random_uniform_fill(n_units, input_distributions, volume_factor=3, wid=None):
    """not proper for elongated input spaces"""
    if wid is None:
        vol = np.product(
            list(np.diff(distr.interval(1)) for distr in input_distributions)
        )
    ...
    for i, j, k, l in u.make_array_ind_iterator(out_fi.shape):
        ...
        if w is None:
            if approx_w:
                ...
                out = min_mse_power(
                    pwr, nu_, dim, max_w=0.5, n_ws=200, lambda_deviation=lam
                )
                w = out[-2]
        out_w[i, j, k, l] = w
...
def get_lattice_uniform_pop(
    ...
    **kwargs
    ...
    ms, ws = get_output_func_distribution_shapes(n_units_pd, distrs, wid_scaling=1)
    ...
    rf, drf = make_gaussian_vector_rf(
        ms, ws**2, scale, 0, titrate_pwr=titrate_pwr, **kwargs
    )
    noise_distr = sts.multivariate_normal(
        np.zeros(n_units_pd**dims), sigma_n, allow_singular=True
    )
...
def get_random_uniform_pop(
    ...
    **kwargs
    ...
    if use_ramp is not None:
        ...
        rf, drf = make_gaussian_ramp_vector_rf(
            ms, uvs, use_ramp, ws, scale, 0, titrate_pwr=titrate_pwr, max_r=1, **kwargs
        )
    else:
        rf, drf = make_gaussian_vector_rf(
            ms, ws, scale, 0, titrate_pwr=titrate_pwr, **kwargs
        )
...
def visualize_random_rf_responses(
    resp,
    cents,
    vis_dims=(0, 1),
    cmap="Blues",
    ...
    for i, r in enumerate(resp):
        ...
        if r > plot_thr:
            plotted += 1
            ax.plot(*pt, "o", color=cmap(r), alpha=r)
    # print(plotted)
    if plot_stim is not None:
        for i, ps in enumerate(plot_stim):
            pt = ps[vis_dims]
            ax.plot(*pt, "o", color=stim_color, markersize=ms)
    ax.set_xlim([0, 1])
    ax.set_ylim([0, 1])
    if len(vis_dims) == 3:
        ax.set_zlim([0, 1])
...
def ml_decode_rf(reps, func, dim, init_guess=None):
    if init_guess is None:
        init_guess = np.zeros((reps.shape[0], dim)) * 0.5
    ...
    out = sopt.basinhopping(
        f, init_guess, niter=100, minimizer_kwargs=dict(bounds=bounds)
    )
    x_opt = np.reshape(out.x, init_guess.shape)
    return x_opt


def brute_decode_decouple(
    reps, func, dim, dim_i=(0,), n_gran=200, init_guess=None, **kwargs
):
    guesses = np.ones((n_gran ** len(dim_i), dim)) * np.nan
    all_guess = np.array(list(it.product(np.linspace(0, 1, n_gran), repeat=len(dim_i))))
...
def brute_decode_rf(reps, func, dim, n_gran=200, init_guess=None, **kwargs):
    guesses = np.array(list(it.product(np.linspace(0, 1, n_gran), repeat=dim)))
...
def _thresh_integrate(mu_p, std_p, sigma_n=1):
    ...
    def integ_func_alt(d):
        ...
        cumu = 0.5 + (1 / np.sqrt(np.pi)) * (-sd + (sd**3) / 3)
        return prob * cumu

    return sint.quad(integ_func, 0, np.inf)


def _approx_thresh_integrate(mu_p, std_p, sigma_n=1, bins=500, lam=3, ret_all=False):
...
def get_ws_range(total_pwr, n_units, dim_, n_ws=100, lambda_dev=2):
    ...
    for i, w in enumerate(ws):
        ...
        out = random_uniform_fi_var(n_units, w, dim_, scale=rescale, ret_pieces=True)
        fi_v, p_v1[i], p_v2[i], p_m2[i] = out
        fi_mm[i] = fi[0, 0] - lambda_dev * np.sqrt(fi_v[0, 0])
        out = compute_threshold_err_prob(
            total_pwr, n_units, dim_, w, resp_scale=rescale, ret_components=True
        )
...
def min_mse_vec(
    pwr, n_units, dims, wid=None, ret_components=False, n_ws=10000, **kwargs
):
    if wid is None:
        wid = np.linspace(0.001, 0.5, n_ws)
    out = mse_w_range(pwr, n_units, dims, wid=wid, ret_components=True, **kwargs)
    mse, l_mse, nl_mse, nl_prob, ws = out
    min_ind = np.nanargmin(mse)

    out = mse[min_ind]
    if ret_components:
        out = (out, l_mse[min_ind], nl_mse[min_ind], nl_prob[min_ind], wid[min_ind])
    return out
...
def mse_w_range(
    ...
    wid=None,
    n_ws=10000,
    ret_components=False,
):
    if wid is None:
        wid = np.linspace(0.001, 0.5, n_ws)
    fi = random_uniform_fi_vec(pwr, n_units, wid, dims, sigma_n=sigma_n)
    p, em = compute_threshold_vec(pwr, n_units, dims, wid, sigma_n=sigma_n, lam=lam)
    out = (1 - p) / fi + p * em
    if ret_components:
        l_mse = 1 / fi
        nl_mse = em
...
def random_uniform_pwr_var_scale(pwr, n_units, wid, dim):
    ...
    out = random_uniform_pwr_var(n_units, wid, dim, scale=scale, vec=True)
    return out
...
def opt_w_approx(pwr, nu, dim, sigma_n=1, def_w=0.01, lam=2):
    sn2 = sigma_n**2
    spi = np.sqrt(np.pi)

    b_num = 3 * (2 ** (dim + 3)) * (np.pi ** (dim / 2)) * sigma_n
    inner_denom = (
        8 * sn2 * (def_w ** (dim / 2)) * np.sqrt(nu) * ((np.pi * 2) ** dim / 4)
    )
    b_denom = (
        dim
        * ss.gamma(dim / 2 + 1)
        * ((1 / np.sqrt(pwr)) + lam * np.sqrt(pwr) / inner_denom)
    )
...
def get_thresh_prob(pwr, n_units, dim, w, **kwargs):
    ...
    return compute_threshold_err_prob(
        pwr, n_units, dim, w, resp_scale=rescale, **kwargs
    )


def compute_threshold_err_prob(
    pwr,
    n_units,
    dim,
    w_opt,
    ...
    mu_p = np.sqrt(2 * pwr)
    std_p = np.sqrt(2 * random_uniform_pwr_var(n_units, w_opt, dim, scale=resp_scale))
    v_std = np.sqrt(random_uniform_pwr_var(n_units, w_opt, dim, scale=resp_scale))
...
def emp_rf_decoding(
    ...
    **kwargs
    ...
    packs = np.zeros((len(total_pwr), len(n_units), len(dims), n_pops), dtype=object)
    configs = {}
    for ind in u.make_array_ind_iterator(packs.shape):
        ...
        if conf_ind is None:
            out = max_fi_power(
                ...
                int(dims[di]),
                sigma_n=sigma_n,
                use_min_func=_min_mse_func,
            )
            fi, _, _, w_opt, rescale_opt = out
            tp, tm = compute_threshold_err_prob(
                total_pwr[pi],
                n_units[ni],
                dims[di],
                w_opt,
    ...
    def _emp_rf_pop(stim_i, rf_i, noise_i):
        ...
        samps_i = stim_i.rvs(n_samps_per_pop, random_state=random_state)
        rep_i = rf_i(samps_i) + noise_i.rvs(n_samps_per_pop, random_state=random_state)
        if give_guess:
            init = samps_i
        else:
            init = None
        dec_i = brute_decode_rf(rep_i, rf_i, samps_i.shape[1], init_guess=init)
        mse_i = (samps_i - dec_i) ** 2
        return mse_i

    par = jl.Parallel(n_jobs=n_jobs, backend="loky")
    out = par(
        jl.delayed(_emp_rf_pop)(*packs[ind])
        for ind in u.make_array_ind_iterator(packs.shape)
    )
...
def _taylor_pwr_var(wid):
    ...
    return ((0.5 * np.sqrt(np.pi) * wid) ** 2) * (
        4 - wid * (4 + np.pi) / np.sqrt(np.pi) + (np.pi / 8) * wid**2
    )
...
def random_uniform_pwr_var(
    n_units, wid, dims, scale=1, use_taylor=False, use_approx=True, vec=False
):
    pwr = random_uniform_pwr(n_units, wid, dims, scale=scale, vec=vec)
    a = scale**4
    b = np.sqrt(np.pi / 2) * wid * ss.erf(np.sqrt(2) / wid) - 0.5 * (wid**2) * (
        1 - np.exp(-2 / (wid**2))
    )
    if use_taylor:
...
def _min_mse_func_nostd(
    w, n_units=None, dims=None, total_pwr=None, lambda_deviation=None
):
    w = w[0]
    ...
    prob, em = compute_threshold_err_prob(
        total_pwr, n_units, dims, w, resp_scale=rescale
    )
...
def _min_mse_func(
    ...
    pwr_pre = random_uniform_pwr(n_units, w, dims, scale=1)
    rescale = np.sqrt(total_pwr / pwr_pre)
    fi = random_uniform_fi(n_units, w, dims, scale=rescale, sigma_n=sigma_n)
    fi_var = random_uniform_fi_var(n_units, w, dims, scale=rescale, sigma_n=sigma_n)
...
def _min_func(w, n_units=None, dims=None, total_pwr=None, lambda_deviation=None):
    w = w[0]
    pwr_pre = random_uniform_pwr(n_units, w, dims, scale=1)
    rescale = np.sqrt(total_pwr / pwr_pre)
    fi = random_uniform_fi(n_units, w, dims, scale=rescale)
...
def max_fi_power(
    ...
    if use_w is not None:
        ...
        res = sopt.basinhopping(
            min_func, (0.5,), niter=n_iters, minimizer_kwargs=minimizer_kwargs, T=T
        )
        w_opt = res.x[0]
    elif opt_kind == "brute":
        ...
        raise TypeError(
            'opt_kind must be one of "basinhop", "brute", or ' '"peak_finding"'
        )
    pwr_pre = random_uniform_pwr(n_units, w_opt, dims, scale=1)
    rescale_opt = np.sqrt(total_pwr / pwr_pre)
    fi = random_uniform_fi(n_units, w_opt, dims, scale=rescale_opt, print_=False)
...
def random_uniform_unit_mean(wid, dims, scale=1):
    b = np.sqrt(2 * np.pi) * wid * ss.erf(1 / (np.sqrt(2) * wid)) - 2 * (wid**2) * (
        1 - np.exp(-1 / (2 * wid**2))
    )
    if u.check_list(wid):
        c = np.product(b)
...
def random_uniform_pwr(n_units, wid, dims, scale=1, vec=True):
    a = scale**2
    b = np.sqrt(np.pi) * wid * ss.erf(1 / wid) - (wid**2) * (
        1 - np.exp(-1 / (wid**2))
    )
    if (not vec) and u.check_list(wid):
        c = np.product(b)
...
def random_uniform_scale_vec(pwr, n_units, wid, dims, modules=1):
    denom1 = np.sqrt(np.pi) * wid * ss.erf(1 / wid) - (wid**2) * (
        1 - np.exp(-1 / (wid**2))
    )
    denom2 = (modules - 1) * (
        np.sqrt(np.pi * 2) * wid * ss.erf(1 / (np.sqrt(2) * wid))
...
def random_uniform_fi_vec(pwr, n_units, wid, dims, sigma_n=1):
    ...
    wid_i2 = wid**2
    wid_i = wid
    b_pre = np.sqrt(np.pi) * wid * ss.erf(1 / wid) - (wid**2) * (
        1 - np.exp(-1 / (wid**2))
    )
    b = b_pre ** (dims - 1)
    ...
    c = (
        0.5
        * wid_i2
        * (np.sqrt(np.pi) * wid_i * ss.erf(1 / wid_i) - 2 * np.exp(-1 / wid_i2))
    )
...
def _non_deriv_terms(wid, m):
    return (0.5 * np.sqrt(np.pi) * wid * (ss.erf((1 - m) / wid) + ss.erf(m / wid))) ** 2
...
def ni_deriv_term(w, approx=False):
    ...
    a = (
        0.25
        * (w**2)
        * (stwo * spi * w * ss.erf(stwo / w) - 4 * np.exp(-2 / (w**2)))
    )
    b = (
        0.5
        * w
    ...
    c = (
        -spi
        * (w**3)
        * (ss.erf(1 / w) - stwo * np.exp(-1 / (2 * w**2)) * ss.erf(1 / (stwo * w)))
    )
    d = (
        -0.5
        * spi
        * (w**3)
        * (stwo * ss.erf(stwo / w) - 2 * np.exp(-1 / (w**2)) * ss.erf(1 / w))
    )
    ...
    k = (
        ...
        * (w**2)
        * (
            (2 * w / spi) * np.exp(-1 / (w**2)) * ss.erf(1 / w)
            + ss.erf(1 / w) ** 2
...
def _wid_rel_integ(wid, n_samps=1000):
    func = (
        lambda diff: (2 - 2 * diff)
        * (diff**4)
        * np.exp(-2 * diff**2 / wid**2)
        / (wid**4)
    )
    out, err = sint.quad(func, 0, 1)
    return out
...
def _wid_full_integ2(wid):
    ...
    def func(z_i, z_j1, z_j2):
        out_i = (2 - 2 * z_i) * (
            (z_i**2) * np.exp(-(z_i**2) / wid_i**2) / (wid_i**2)
        ) ** 2
...
def random_uniform_fi_var_simp(n_units, w, dims, total_pwr, sigma_n=1):
    ...
    e = (
        ...
        + 8 * np.exp(-1 / (4 * w**2)) * w
        + 4 * spi * ss.erf(1 / (2 * w))
        + 8 * stwo * np.exp(-1 / (2 * w**2)) * w * ss.erf(1 / (stwo * w))
    )
...
def random_uniform_fi_var_pwr(n_units, pwr, wid, dims, **kwargs):
    ...
    return random_uniform_fi_var(n_units, wid, dims, scale=rescale, **kwargs)[0, 0]


def random_uniform_fi_var(
    n_units,
    wid,
    dims,
    scale=1,
    ...
    fi_mean = random_uniform_fi(n_units, wid, dims, scale=scale, sigma_n=sigma_n)
    fi_v2 = np.zeros_like(fi_mean)
    b_pre = np.sqrt(np.pi / 2) * wid * ss.erf(np.sqrt(2) / wid) - 0.5 * (wid**2) * (
        1 - np.exp(-2 / (wid**2))
    )
    if ret_pieces:
    ...
    for i in range(dims):
        ...
        fi_v2[i, i] = pref * (
            (n_units) * fiv / wid_i4 + (n_units - 1) * n_units * off_diag
        )
        if ret_pieces:
    ...
    out = fi_var
    if ret_pieces:
        out = (fi_var, np.mean(piece_fiv), np.mean(piece_off), np.mean(piece_fim))
    else:
        out = fi_var
    return out


def get_output_func_distribution_shapes(n_units_pd, input_distributions, wid_scaling=1):
...
def get_distribution_gaussian_resp_func(
    n_units_pd,
    input_distributions,
    scale=1,
    baseline=0,
    ...
    ms, ws = get_output_func_distribution_shapes(
        n_units_pd, input_distributions, wid_scaling=wid_scaling
    )
    if random_widths:
        rng = np.random.default_rng()
        min_w = np.min(ws)
    ...
    resp_func, d_resp_func = make_gaussian_vector_rf(ms, ws, scale, baseline)
    return resp_func, d_resp_func, ms, ws


def make_gaussian_ramp_vector_rf(
    cents,
    uvs,
    ramp_dim,
    sizes,
    ...
    drfs = ft.partial(
        eval_gaussian_ramp_vector_deriv,
        cents=cents,
        sizes=sizes,
        scale=new_scale,
        baseline=baseline,
...
def make_ramp_vector_rf(num, extent, scale, baseline, sub_dim=None):
    ...
    rfs = ft.partial(
        eval_ramp_vector_rf, extent=extents, scale=scale, baseline=baseline
    )
    drfs = ft.partial(
        eval_ramp_vector_deriv, extent=extents, scale=scale, baseline=baseline
    )
    return rfs, drfs
...
def make_2dgaussian_rf(cent, xsize, ysize, scale, baseline):
    def gaussian2d_rf(coords):
        ...
        out = (
            ...
            * np.exp(
                -(
                    ((xs - cent[0]) ** 2) / (2 * xsize**2)
                    + ((ys - cent[1]) ** 2) / (2 * ysize**2)
                )
            )
            + baseline
        )
...
def fim(pts, deriv_func, noise_var=None, cov=None):
    ...
    for mc in mat_combs:
        i, j = mc
        val = np.dot(d_resps[i][0, :, 0], np.dot(inv_cov, d_resps[j][0, :, 0].T))
        fim_mat[i, j] = val
        fim_mat[j, i] = val
    return fim_mat
...
def construct_rf_pop(
    ...
    if return_objects:
        rfs = [rf_func(c, rf_sizes, resp, base, sub_dim=sub_dim) for c in cents]
    else:
        rfs = [(rf_func, (c, rf_sizes, resp, base, sub_dim)) for c in cents]
    return rfs
