...
def zscore_tc_shape(
    ...
    if fit_pop is not None:
        scale = scaler().fit(fit_pop[..., 0])
    else:
        scale = None
    for i in range(pop.shape[-1]):
        if scale is not None:
            new_pop[..., i] = scale.transform(pop[..., i])
        else:
...
class ModelPipelineCombinedTC(skb.BaseEstimator):
    def __init__(self, model=None, pipe=None, **kwargs):
        self.model = model
        self.kwargs = kwargs
        if pipe is None:
            pipe = _make_model_pipeline(self.model, **self.kwargs)
        self.pipe = pipe

    def _format_y(self, X, y=None):
        if y is None:
            y_tr = None
        elif len(y.shape) > 1 and y.shape[-1] == X.shape[-1]:
            y_tr = u.combine_dimensions(y, 0, -1)
        else:
            y_tr = u.combine_dimensions(np.ones_like(X) * y[:, None, None], 0, -1)
        return y_tr

    def _format_x(self, X):
        return u.combine_dimensions(X, 0, -1)
    ...
    def fit(self, X, y=None, **kwargs):
        ...
        y_tr = self._format_y(X, y)
        pipe = self.pipe.fit(X_tr, y_tr, **kwargs)
        out = ModelPipelineCombinedTC(self.model, pipe=pipe, **self.kwargs)
        self.pipe = pipe
        return out

    def fit_predict(self, X, **kwargs):
        X_use = self._format_x(X)
        return self._unformat_x(self.pipe.fit_predict(X_use), X.shape[-1])
...
def make_model_pipeline(*args, single_tc=False, tc=False, **kwargs):
    if tc:
        out = ModelPipelineTC(*args, **kwargs)
    elif single_tc:
        out = ModelPipelineCombinedTC(*args, **kwargs)
    else:
...
class MultiOutputWrapper:
    def __init__(self, model, moc, **kwargs):
        self.moc = moc(model, **kwargs)
    ...
    def transform(self, X):
        dfs = []
        for est in self.moc.estimators_:
            dfs.append(est.transform(X))
        return np.stack(dfs, axis=1)


class MultiOutputClassifierWrapper(MultiOutputWrapper):
    def __init__(self, model, **kwargs):
        super().__init__(model, skout.MultiOutputClassifier, **kwargs)


class MultiOutputRegressorWrapper(MultiOutputWrapper):
    def __init__(self, model, **kwargs):
        super().__init__(model, skout.MultiOutputRegressor, **kwargs)
...
def _make_model_pipeline(
    ...
    multioutput=False,
    oversample=False,
    oversampler=imb_os.RandomOverSampler,
    undersample=False,
    ...
    **kwargs,
    ...
    pipe_steps = []
    if norm:
        pipe_steps.append(skp.StandardScaler(with_std=with_std))
    if impute_missing:
    ...
    if model is not None:
        if multioutput:
            if regression:
                m = MultiOutputRegressorWrapper(model(**kwargs))
            else:
...
def _fit_model_preds(data, labels, estimators):
    ...
    out = np.zeros(out_shape, dtype=labels.dtype)
...
class BalancedCV:
    ...
    @classmethod
    def with_cv(cls, cv, **kwargs):
        return cls(cv_use=cv, **kwargs)
...
def cv_wrapper(
    ...
    **kwargs,
    ...
    splitter = cv_type(n_folds, **cv_kwarg)
    ...
    for i, te_inds in enumerate(out["indices"]["test"]):
        est = out["estimator"][i]
...
def fold_skl_continuous_shape(
    ...
    **kwargs,
    ...
    return fold_skl_continuous(c_flat, labels, folds_n, c_gen=c_gen, **kwargs)
...
def fold_skl_continuous(
    ...
    **model_kwargs,
    ...
    if test_prop is None:
        test_prop = 1 / folds_n
    if norm_targets:
        oned = len(labels.shape) == 1
        if oned:
            labels = labels[:, None]
        labels = skp.StandardScaler().fit_transform(labels)
        if oned:
            labels = labels[:, 0]
...
def nominal_fold(
    ...
    **kwargs,
    ...
    outs = []
    if not label_tc:
        labels = np.stack((labels,) * x_len, axis=-1)
        if c_gen is not None:
            l_gen = np.stack((l_gen,) * x_len, axis=-1)
    ...
    for j in range(x_len):
        ...
        if c_gen is not None:
            if time_accumulate:
                ...
                gen_data = c_gen[..., j].T
            out_j["predictions_gen"] = _fit_model_preds(
                gen_data, l_gen[..., j], out_j["estimator"]
            )
            out_j["projection_gen"] = _fit_model_proj(
                gen_data, l_gen[..., j], out_j["estimator"]
            )
            out_j["score_gen"] = _eval_fit_models(
                gen_data,
                l_gen[..., j],
                out_j["estimator"],
                scoring=scoring,
        ...
        if time_generalization:
            for k in range(x_len):
                out_j["time_generalization"] = _eval_fit_models(
                    c_flat[..., k].T,
                    labels[..., k],
                    out_j["estimator"],
                    scoring=scoring,
                ...
                if c_gen is not None:
                    out_j["time_generalization_gen"] = _eval_fit_models(
                        c_gen[..., k].T,
                        l_gen[..., k],
                        out_j["estimator"],
                        scoring=scoring,
        ...
        outs.append(out_j)
    out = u.aggregate_dictionary(outs, combine_axis=-1)
    out["score"] = out.pop("test_score")
    out["estimators"] = out.pop("estimator")
    if mean:
        out["score"] = np.mean(out["score"], axis=0)
        if "score_gen" in out.keys():
            out["score_gen"] = np.mean(out["score_gen"], axis=0)
    return out
...
def apply_estimators(estimators, pop, labels, transpose=True):
    ...
    for i, j in u.make_array_ind_iterator(estimators.shape):
        ...
        if transpose:
            pop_j = pop_j.T
        out[i, j] = est_ij.score(pop_j, labels)
    return out
...
def _fit_and_score_collapse_tc(
    ...
    **kwargs,
    ...
    if label_tc:
        y_tr_flat = np.concatenate(list(y_tr[..., j] for j in range(x_tr_len)), axis=0)
    else:
        y_tr_flat = np.tile(y_tr, x_tr_len)
        y_te = np.stack((y_te,) * x_len, axis=-1)
        y_tr = np.stack((y_tr,) * x_len, axis=-1)
    est.fit(X_tr_flat, y_tr_flat)

    test_scores = np.array(
        list(est.score(X_te[..., j], y_te[..., j]) for j in range(x_len))
    )
    train_scores = np.array(
        list(est.score(X_tr[..., j], y_tr[..., j]) for j in range(x_len))
    )
    test_proj = np.array(
        list(est.decision_function(X_te[..., j]) for j in range(x_len))
    )
    train_proj = np.array(
        list(est.decision_function(X_tr[..., j]) for j in range(x_len))
    )
    out_dict = {
        ...
        "test_targ": y_te,
        "train_targ": y_tr,
        "indices": {"train": tr_inds, "test": te_inds},
    }
    if return_estimator:
...
class LinearSVCWrapper(sksvm.LinearSVC):
    def transform(self, X):
        return self.decision_function(X) / np.sqrt(np.sum(self.coef_**2))


class SVCWrapper(sksvm.SVC):
    def transform(self, X):
        if self.kernel == "linear":
            norm = np.sqrt(np.sum(self.coef_**2))
        else:
            norm = 1
        return self.decision_function(X) / norm


def relevant_projection_across_time(X, y, **kwargs):
    return targeted_dimensionality_reduction(
        X, y, model=LinearSVCWrapper, cv=skms.LeaveOneGroupOut(), **kwargs
    )


def targeted_dimensionality_reduction(
    X,
    y,
    n_folds=10,
    cv=None,
    ...
    balance_targets=False,
):
    if len(y.shape) > 1 and use_multiclass:
        _, y_use = np.unique(y, axis=0, return_inverse=True)
        multioutput = False
        n_targs = len(np.unique(y_use)) - 1
    else:
        y_use = y
        multioutput = len(y.shape) > 1
        n_targs = y.shape[1] if multioutput else 1
    if cv is None:
        cv = skms.GroupShuffleSplit(n_folds, test_size=0.1)
    if est is None:
        est = make_model_pipeline(model=model, pca=pre_pca, multioutput=multioutput)
    if balance_targets:
        cv = BalancedCV.with_cv(cv)
    assert len(X.shape) == 3
    if time_mask is None:
        time_mask = np.ones(X.shape[-1], dtype=bool)
    X_tr_time = X[..., time_mask]
    n_ts = X_tr_time.shape[-1]
    n_trls = X_tr_time.shape[0]
    groups = np.arange(n_trls)

    X_full_time = np.concatenate(list(X_tr_time[..., j] for j in range(n_ts)), axis=0)
    groups_full = np.tile(groups, n_ts)
    y_use_full = np.tile(y_use, (n_ts, 1) if multioutput else n_ts)
    ...
    fit_est = out["estimator"]
    indices = out["indices"]["test"]
    n_te_trls = len(np.unique(groups_full[indices[0]]))
    X_proj = np.zeros(
        (
            len(indices),
            n_te_trls,
    ...
    y_proj = np.zeros((len(indices), n_te_trls, n_targs))
    te_score = np.zeros((len(indices), n_ts))
    for i, fe in enumerate(fit_est):
        trls_test = np.unique(groups_full[indices[i]])
        X_te = X[trls_test]
        y_proj[i] = y[trls_test]
        X_proj[i] = np.stack(
            list(fe.transform(X_te[..., j]) for j in range(n_ts)), axis=-1
        )
        te_score[i] = list(
            fe.score(X_te[..., j], y_use[trls_test]) for j in range(n_ts)
        )
    out["test_projection"] = X_proj
    out["test_targets"] = y_proj
    out["test_score"] = te_score
    return out
...
def generalize_skl_shape(c_flat, targ_labels, cond_labels, folds_n, **kwargs):
    u_cls = np.unique(cond_labels)
    outs = []
    for cl in u_cls:
        mask = cond_labels == cl
        c_tr = c_flat[mask]
        targ_tr = targ_labels[mask]
        c_gen = c_flat[np.logical_not(mask)]
        l_gen = targ_labels[np.logical_not(mask)]
        outs.append(
            fold_skl_shape(c_tr, targ_tr, folds_n, c_gen=c_gen, l_gen=l_gen, **kwargs)
        )
    full_dict = u.aggregate_dictionary(outs)
    full_dict["conditioned_label"] = u_cls
    return full_dict
...
def fold_skl_flat(
    ...
    **model_kwargs,
    ...
    pipe = make_model_pipeline(
        ...
        pca=pre_pca,
        multioutput=len(labels.shape) > 1 and not label_tc,
    ...
    if collapse_time:
        ...
        if mean:
            score = np.nan * np.ones(c_flat.shape[-1])
        else:
            score = np.nan * np.ones((folds_n, c_flat.shape[-1]))
        out = {"score": score}
        if c_gen is not None:
            out["score_gen"] = score
    return out
